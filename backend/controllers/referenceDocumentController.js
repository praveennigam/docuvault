...
export const createOrUpdateReferenceDocument = async (req, res) => {
  try {
    ...
    // ✅ Validate all selected documents exist + auto fetch fields + type
    const enrichedDocs = [];
    for (const d of selectedDocs) {
      const docId = d.document._id || d.document;
      if (!mongoose.Types.ObjectId.isValid(docId))
      ...
      // 🟢 Add type and fields automatically if not given
      enrichedDocs.push({
        document: new mongoose.Types.ObjectId(docExists._id),
        documentType: docExists.type, // ✅ Added type
        requiredFields:
          d.fields && d.fields.length > 0
            ? d.fields
            : docExists.fields.map((f) => ({
                key: f.key,
                label: f.label,
              })),
      });
    ...
    if (refDoc) {
      ...
      enrichedDocs.forEach((d) => {
        const docId = d.document.toString();
        if (!existingIds.includes(docId)) {
          refDoc.documents.push({
            document: new mongoose.Types.ObjectId(d.document),
            documentType: d.documentType, // ✅ Added type here
            requiredFields: d.requiredFields || [],
          });
        }
      });
    ...
    refDoc = new ReferenceDocument({
      label,
      category,
      documents: enrichedDocs, // ✅ Includes auto type + fields
      conditions: conditions || [],
      protocols: protocolIds,
    });
...
export const cloneReferenceDocument = async (req, res) => {
  try {
    ...
    const clonedData = {
      ...
      documents: refDoc.documents.map((d) => ({
        document: d.document,
        documentType: d.documentType, // ✅ included in clone
        requiredFields: d.requiredFields || [],
      })),
      conditions: Array.isArray(refDoc.conditions)
        ? [...refDoc.conditions]
